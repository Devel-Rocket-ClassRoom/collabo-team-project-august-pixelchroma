...
public static class ScreenPrefabBootstrap
{
    ...
    private static void InstallForScene(Scene scene)
    {
        ...
        if (scene.name == "0.Tilte" && catalog.TitleScreenPrefab != null)
            Object.Instantiate(catalog.TitleScreenPrefab).name = "UI_타이틀 (PlayHere)";
        else if (scene.name == "1.MainMenu" && catalog.HomeScreenPrefab != null)
            Object.Instantiate(catalog.HomeScreenPrefab).name = "UI_메인 로비 (PlayHere)";
        else if (scene.name == "5.CharList" && catalog.AgentListScreenPrefab != null)
            Object.Instantiate(catalog.AgentListScreenPrefab).name = "UI_요원 리스트 (PlayHere)";
        else if (scene.name == "2.Chapter Select" && catalog.ChapterScreenPrefab != null)
            Object.Instantiate(catalog.ChapterScreenPrefab).name = "UI_챕터 선택 (PlayHere)";
        else if (scene.name == "3.Stage List" && catalog.StageScreenPrefab != null)
            Object.Instantiate(catalog.StageScreenPrefab).name = "UI_스테이지 선택 (PlayHere)";
        else if (scene.name == "6.Status List" && catalog.AgentStatusScreenPrefab != null)
            Object.Instantiate(catalog.AgentStatusScreenPrefab).name = "UI_요원 상세 (PlayHere)";
        else
            return;
